...
def run_proposal_agent(query_engine, rfp_text_summary):
    ...
    with st.status("🔍 1단계: RFP 핵심 요구사항 분석 중...", expanded=True) as status:
        ...
        try:
            ...
            analysis_result = {
                ...
                "sections": ["사업 이해", "수행 계획", "조직 구성", "기대 효과"]
            }
    ...
    with st.status("📋 2단계: 제안서 목차 구성 중...", expanded=True) as status:
        outline_prompt = PromptTemplate.from_template(
            ...
            출력 형식:
            {{"sections": ["1. 사업 이해 및 추진 방향", "2. 교육 프로그램 구성", "3. 조직 및 인력 투입 계획"]}}"""
        )
        try:
    ...
    for loop_count in range(1, max_loops + 1):
        ...
        # 4단계: 요구사항 충족 검토
        with st.status(f"⚖️ 4단계: 요구사항 충족률 검토 중...", expanded=True) as status:
        ...
        if uploaded_file:
            if not st.session_state.company_info:
                ...
                if st.button("🔍 RFP 인덱싱 시작", type="primary"):
                    with st.spinner("PDF를 분석하고 벡터 DB에 저장 중입니다... (1~2분 소요)"):
                        try:
                            with tempfile.TemporaryDirectory() as tmp_dir:
                                ...
                                for doc in documents:
                                    doc.metadata["rfp_name"] = uploaded_file.name
    ...
    if st.session_state.index is None:
        ...
        if st.session_state.proposal_sections:
                            ...
                            if st.button("🔄 재생성", key=f"regen_{section}"):
                                with st.spinner(f"{section} 재생성 중..."):
                                    try:
                                        ...
                                        regen_chain = regen_prompt | llm
                                        new_text = regen_chain.invoke({"ctx": rag_resp, "sec": section})
                                        st.session_state.proposal_sections[section] = new_text.content
                                        st.rerun()
                                    except Exception as e:
                                        st.error(f"재생성 오류: {e}")
            ...
            try:
                ...
                d_col1.error(f"Word 생성 오류: {e}")

            # PDF 다운로드 (한글 미지원 안내)
            d_col2.info("PDF 다운로드는 Word 파일을 PDF로 변환하여 사용하세요.")
